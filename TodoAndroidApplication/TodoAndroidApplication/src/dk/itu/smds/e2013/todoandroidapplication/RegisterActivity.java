...
import dk.itu.smds.e2013.todoandroidapplication.messageEndpoint.MessageEndpoint;
import dk.itu.smds.e2013.todoandroidapplication.messageEndpoint.model.CollectionResponseMessageData;
import dk.itu.smds.e2013.todoandroidapplication.messageEndpoint.model.MessageData;
import dk.itu.smds.e2013.todoandroidapplication.todoendpoint.Todoendpoint;
import dk.itu.smds.e2013.todoandroidapplication.todoendpoint.model.CollectionResponseTodo;
import dk.itu.smds.e2013.todoandroidapplication.todoendpoint.model.Todo;
...
public class RegisterActivity extends Activity {
  ...
  private Todoendpoint  todoEndpoint = null ;
...
//    messageEndpoint = CloudEndpointUtils.updateBuilder(endpointBuilder).build();
     ...
     * build the messaging endpoint so we can access old messages via an endpoint call
     */
    // Rao: 20131103: Change the MessageEndpoint to Todoendpoint to receive messages about 
    // Todo items
    ...
    todoEndpoint = CloudEndpointUtils.updateBuilder(endpointBuilder).build();
    ...
    if (intent.getBooleanExtra("gcmIntentServiceMessage", false)) {
      ...
      if (intent.getBooleanExtra("registrationMessage", false)) {
        ...
        // new QueryMessagesTask(this, messageEndpoint).execute();
    	  ...
    	  // Rao: 20131103: Create a new QueryTodoTask to get the todo items from Google app engine
    	  new QueryTodoTask(this, todoEndpoint).execute();
  ...
  /* Rao: 20131103
   * this task will ask the App Engine backend for the last 5 Todo items
   */
  private class QueryTodoTask 
      extends AsyncTask<Void, Void, CollectionResponseTodo> {
    Exception exceptionThrown = null;
    Todoendpoint todoEndpoint;
    

    public QueryTodoTask(Activity activity, Todoendpoint todoEndpoint) {
    ...
    protected CollectionResponseTodo doInBackground(Void... params) {
      try {
        CollectionResponseTodo todoItems = 
        		todoEndpoint.listTodo().setLimit(5).execute();
        return todoItems;
      } catch (IOException e) {
        exceptionThrown = e;
        return null;
    ...
    protected void onPostExecute(CollectionResponseTodo todos) {
      // Check if exception was thrown
      if (exceptionThrown != null) {
        Log.e(RegisterActivity.class.getName(), 
            "Exception when listing Messages", exceptionThrown);
        showDialog("Failed to retrieve the last 5 messages from " +
        		"the endpoint at " + todoEndpoint.getBaseUrl() +
        		", check log for details");
      }
      else {
        TextView messageView = (TextView) findViewById(R.id.msgView);
        messageView.setText("Todo items read from " + 
        		todoEndpoint.getBaseUrl() + ":\n");
        for(Todo todo : todos.getItems()) {
          messageView.append(todo.getId() + "\n");
        }
      }
    }   
  }
